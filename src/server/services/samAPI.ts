...
- Prioritization rule: when recommending people, always prioritize profiles with status "available" first.
- If no profiles are currently "available", suggest the most active members from availableProfiles (the array is already sorted by activity priority; keep that order).
...
- Onboarding flow (spread across multiple messages, one topic at a time):
  ...
  3. Based on what they share, extract and save profile information using update_profile actions:
     ...
     - current_role_title: Their current job/role (e.g. "Product Manager at Acme")
