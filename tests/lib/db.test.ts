...
let bootstrapSnapshot: BootstrapPayload | undefined;

beforeAll(() => {
  if (typeof window !== 'undefined') {
    bootstrapSnapshot = window.__HUMANCHAT_BOOTSTRAP__;
  }
});

beforeEach(async () => {
  if (typeof window !== 'undefined') {
    window.__HUMANCHAT_BOOTSTRAP__ = undefined;
  }
  if (db.isOpen()) {
    db.close();
  }
...
afterAll(() => {
  ...
  if (typeof window !== 'undefined') {
    window.__HUMANCHAT_BOOTSTRAP__ = bootstrapSnapshot;
  }
});
