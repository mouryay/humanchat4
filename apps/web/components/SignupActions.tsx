...
type SignupStep = 'persona' | 'access' | 'context';
...
export default function SignupActions() {
  ...
  const [step, setStep] = useState<SignupStep>('persona');
  ...
  useEffect(() => {
    if (selectedPersona && step === 'persona') {
      setStep('access');
      requestAnimationFrame(() => {
        emailInputRef.current?.focus();
      });
    }
  }, [selectedPersona, step]);
  ...
  return (
    <div className="mt-4 space-y-6">
      <div className="flex items-center gap-2 text-xs uppercase tracking-[0.35em] text-white/50">
        <span className={step === 'persona' ? 'text-white' : ''}>1. Intent</span>
        <div className="h-px flex-1 bg-white/20" />
        <span className={step === 'access' ? 'text-white' : ''}>2. Access</span>
        <div className="h-px flex-1 bg-white/20" />
        <span className={step === 'context' ? 'text-white' : ''}>3. Context</span>
      </div>

      <div className="space-y-4">
        <p className="text-sm uppercase tracking-[0.35em] text-white/60">How are you joining?</p>
          ...
          {personaOptions.map((option) => {
            ...
            return (
              <button
                key={option.id}
                type="button"
                ref={option.id === personaOptions[0].id ? primaryButtonRef : undefined}
        ...
        {!selectedPersona && <p className="text-xs text-slate-300">Pick one to unlock the next step.</p>}
      </div>
